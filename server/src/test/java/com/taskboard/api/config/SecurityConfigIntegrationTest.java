...
/**
 * Integration tests for Security Configuration Tests Spring Security 6.5 features including DPoP,
 * WebAuthn, One-Time Token, and OAuth2
 */
@SpringBootTest
@AutoConfigureWebMvc
...
class SecurityConfigIntegrationTest {
  ...
  void testWebAuthnEndpointsAccessible() throws Exception {
    mockMvc.perform(get("/api/webauthn/register/challenge")).andExpect(status().isOk());
  }

  @Test
  void testOneTimeTokenEndpointsAccessible() throws Exception {
    mockMvc.perform(post("/api/one-time-tokens/generate")).andExpect(status().isOk());
  }
  ...
  void testCorsHeadersPresent() throws Exception {
    mockMvc
        .perform(get("/api/auth/login").header("Origin", "http://localhost:4200"))
