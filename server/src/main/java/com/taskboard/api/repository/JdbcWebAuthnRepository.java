...
/**
 * JDBC repository for WebAuthn credentials and challenges Implements Spring Security 6.5 WebAuthn
...
public interface JdbcWebAuthnRepository extends JpaRepository<WebAuthnCredential, UUID> {
  ...
  @Query(
      "SELECT w FROM WebAuthnCredential w WHERE w.credentialId = :credentialId AND w.isActive = true")
  ...
  @Query(
      "SELECT c FROM WebAuthnChallenge c WHERE c.challenge = :challenge AND c.isUsed = false AND c.expiresAt > :now")
  Optional<WebAuthnChallenge> findActiveChallenge(
      @Param("challenge") String challenge, @Param("now") LocalDateTime now);

  /** Find active challenges for a user */
  @Query(
      "SELECT c FROM WebAuthnChallenge c WHERE c.userId = :userId AND c.isUsed = false AND c.expiresAt > :now")
  List<WebAuthnChallenge> findActiveChallengesByUserId(
  ...
  @Query(
      "UPDATE WebAuthnCredential w SET w.counter = :counter, w.lastUsedAt = :lastUsedAt WHERE w.credentialId = :credentialId")
  void updateCredentialCounter(
