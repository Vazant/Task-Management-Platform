...
public interface OneTimeTokenRepository extends JpaRepository<OneTimeToken, UUID> {
  ...
  @Query(
      "SELECT t FROM OneTimeToken t WHERE t.token = :token AND t.isUsed = false AND t.expiresAt > :now")
  Optional<OneTimeToken> findActiveByToken(
      @Param("token") String token, @Param("now") LocalDateTime now);

  /** Находит активные токены пользователя по цели */
  @Query(
      "SELECT t FROM OneTimeToken t WHERE t.userId = :userId AND t.purpose = :purpose AND t.isUsed = false AND t.expiresAt > :now")
  List<OneTimeToken> findActiveByUserIdAndPurpose(
      @Param("userId") String userId,
      @Param("purpose") OneTimeToken.TokenPurpose purpose,
  ...
  @Query(
      "SELECT COUNT(t) FROM OneTimeToken t WHERE t.userId = :userId AND t.purpose = :purpose AND t.isUsed = false AND t.expiresAt > :now")
  long countActiveByUserIdAndPurpose(
      @Param("userId") String userId,
      @Param("purpose") OneTimeToken.TokenPurpose purpose,
      @Param("now") LocalDateTime now);
  ...
  List<OneTimeToken> findByCreatedAtBetween(
