...
public interface ProjectRepository extends JpaRepository<Project, Long> {
  ...
  List<Project> findByCreatedByAndStatus(
      @Param("createdBy") String createdBy, @Param("status") String status);

  @Query("SELECT p FROM Project p WHERE p.createdBy = :createdBy AND p.name LIKE %:name%")
  List<Project> findByCreatedByAndNameContaining(
      @Param("createdBy") String createdBy, @Param("name") String name);
}
