...
public interface WebAuthnChallengeRepository extends JpaRepository<WebAuthnChallenge, UUID> {
    ...
    Optional<WebAuthnChallenge> findActiveByUserId(@Param("userId") String userId, @Param("now") LocalDateTime now);

    /**
     * Находит активный challenge по значению
     */
    @Query("SELECT w FROM WebAuthnChallenge w WHERE w.challenge = :challenge AND w.isUsed = false AND w.expiresAt > :now")
    Optional<WebAuthnChallenge> findActiveByChallenge(@Param("challenge") String challenge, @Param("now") LocalDateTime now);
     ...
     */
    Optional<WebAuthnChallenge> findByChallengeAndType(String challenge, WebAuthnChallenge.ChallengeType type);
}
