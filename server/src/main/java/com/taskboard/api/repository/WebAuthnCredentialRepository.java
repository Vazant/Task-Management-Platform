...
public interface WebAuthnCredentialRepository extends JpaRepository<WebAuthnCredential, UUID> {
     ...
     */
    @Query("UPDATE WebAuthnCredential w SET w.counter = :counter, w.lastUsedAt = CURRENT_TIMESTAMP WHERE w.credentialId = :credentialId")
    void updateCounterAndLastUsed(@Param("credentialId") String credentialId, @Param("counter") Long counter);
