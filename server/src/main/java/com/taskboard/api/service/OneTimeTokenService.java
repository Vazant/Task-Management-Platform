...
/**
 * Service for managing one-time tokens Provides secure temporary access for various authentication
 * purposes
 */
@Service
@Slf4j
...
public class OneTimeTokenService {
  ...
  @Autowired private OneTimeTokenRepository tokenRepository;
  ...
  public OneTimeToken createEmailVerificationToken(
      final String userId, final int expirationMinutes) {
    return createToken(
        userId, OneTimeToken.TokenPurpose.EMAIL_VERIFICATION, expirationMinutes, null);
  }

  /** Создает одноразовый токен для административного доступа */
  public OneTimeToken createAdminAccessToken(
      final String userId, final int expirationMinutes, final String metadata) {
  ...
  public OneTimeToken createApiAccessToken(
      final String userId, final int expirationMinutes, final String metadata) {
  ...
  public OneTimeToken createEmergencyAccessToken(
      final String userId, final int expirationMinutes, final String metadata) {
    return createToken(
        userId, OneTimeToken.TokenPurpose.EMERGENCY_ACCESS, expirationMinutes, metadata);
  }

  /** Создает одноразовый токен с указанными параметрами */
  public OneTimeToken createToken(
      final String userId,
      final OneTimeToken.TokenPurpose purpose,
      final int expirationMinutes,
      final String metadata) {
    try {
      // Проверяем лимит активных токенов
      long activeTokensCount =
          tokenRepository.countActiveByUserIdAndPurpose(userId, purpose, LocalDateTime.now());
      ...
      log.info(
          "Создан одноразовый токен для пользователя {} с целью {} (истекает через {} минут)",
          userId,
          purpose,
          expirationMinutes);
  ...
  public boolean validateAndUseToken(
      final String tokenValue, final OneTimeToken.TokenPurpose expectedPurpose) {
    try {
      // Находим активный токен
      Optional<OneTimeToken> tokenOpt =
          tokenRepository.findActiveByToken(tokenValue, LocalDateTime.now());
      ...
      log.info(
          "Одноразовый токен успешно использован: {} для пользователя {}",
          tokenValue,
          token.getUserId());
  ...
  public List<OneTimeToken> getActiveTokensForUserAndPurpose(
      final String userId, final OneTimeToken.TokenPurpose purpose) {
  ...
  public void revokeTokensForUserAndPurpose(
      final String userId, final OneTimeToken.TokenPurpose purpose) {
    ...
    do {
      ...
      if (attempts >= maxAttempts) {
        throw new RuntimeException(
            "Не удалось сгенерировать уникальный токен после " + maxAttempts + " попыток");
      }
