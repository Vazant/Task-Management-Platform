...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiResponse<Map<String, String>>> handleValidationExceptions(
    ...
    // Возвращаем конкретные ошибки валидации вместо общего сообщения
    String firstError = errors.values().iterator().next();
    return ResponseEntity.badRequest().body(new ApiResponse<>(errors, firstError, false));
  }
  ...
  public ResponseEntity<ApiResponse<String>> handleGenericException(Exception ex) {
    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
        .body(
            new ApiResponse<>(
                null,
                messageService.getMessage("error.global.unexpected", ex.getMessage()),
                false));
  }
}
