...
/**
 * Контроллер для работы с аватарами пользователей. Поддерживает presigned URL для загрузки и CDN
 * для отдачи.
 */
@RestController
...
public class AvatarController {
  ...
  public ResponseEntity<ApiResponse<AvatarUploadResponse>> generateUploadUrl(
      @Valid @RequestBody AvatarUploadRequest request, Authentication authentication) {
    ...
    try {
      ...
      return ResponseEntity.ok(
          new ApiResponse<>(response, "Upload URL generated successfully", true));
  ...
  public ResponseEntity<ApiResponse<AvatarResponse>> confirmUpload(
      @Valid @RequestBody AvatarConfirmRequest request, Authentication authentication) {
    ...
    try {
      ...
      return ResponseEntity.ok(
          new ApiResponse<>(response, "Avatar upload confirmed successfully", true));
  ...
  public ResponseEntity<ApiResponse<AvatarResponse>> getActiveAvatar(@PathVariable Long userId) {
    try {
      ...
      return ResponseEntity.ok(
          new ApiResponse<>(avatar, "Active avatar retrieved successfully", true));
  ...
  @PreAuthorize("isAuthenticated()")
  public ResponseEntity<ApiResponse<List<AvatarResponse>>> getAllAvatars(
    ...
    try {
      ...
      return ResponseEntity.ok(
          new ApiResponse<>(avatars, "All avatars retrieved successfully", true));
  ...
  public ResponseEntity<ApiResponse<String>> deleteAvatar(
    ...
    try {
      log.info(
          "Avatar deletion request from user: {} for user ID: {}",
      ...
      if (!currentUser.getId().equals(userId)) {
        return ResponseEntity.status(403).body(new ApiResponse<>(null, "Access denied", false));
      }

      avatarService.deleteAvatar(userId);

      return ResponseEntity.ok(
          new ApiResponse<>("Avatar deleted", "Avatar deleted successfully", true));
  ...
  public ResponseEntity<ApiResponse<String>> generateDownloadUrl(@PathVariable Long userId) {
    try {
      ...
      return ResponseEntity.ok(
          new ApiResponse<>(downloadUrl, "Download URL generated successfully", true));
  ...
  @PreAuthorize("isAuthenticated()")
  public ResponseEntity<ApiResponse<AvatarService.AvatarStats>> getAvatarStats(
      @PathVariable Long userId) {
    try {
      ...
      return ResponseEntity.ok(
          new ApiResponse<>(stats, "Avatar stats retrieved successfully", true));
  ...
  @GetMapping("/me")
  @PreAuthorize("isAuthenticated()")
  public ResponseEntity<ApiResponse<AvatarResponse>> getMyActiveAvatar(
