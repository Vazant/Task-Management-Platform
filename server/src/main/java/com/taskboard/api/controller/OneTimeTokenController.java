...
public class OneTimeTokenController {
    ...
    public ResponseEntity<Map<String, Object>> createLoginToken(
        ...
        try {
            ...
            if (!hasPermissionToCreateToken(userId, authentication)) {
                return ResponseEntity.status(403).body(createErrorResponse("Недостаточно прав для создания токена"));
            }
            ...
            return ResponseEntity.badRequest().body(createErrorResponse("Не удалось создать токен для входа"));
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> createPasswordResetToken(
        ...
        try {
            ...
            return ResponseEntity.badRequest().body(createErrorResponse("Не удалось создать токен для сброса пароля"));
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> createAdminAccessToken(
        ...
        try {
            ...
            if (!isAdmin(authentication)) {
                return ResponseEntity.status(403).body(createErrorResponse("Требуются права администратора"));
            }
            ...
            return ResponseEntity.badRequest().body(createErrorResponse("Не удалось создать токен административного доступа"));
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> validateToken(
            ...
            @RequestParam OneTimeToken.TokenPurpose purpose) {
    ...
    public ResponseEntity<Map<String, Object>> getTokenInfo(@PathVariable String token) {
        try {
            ...
            return ResponseEntity.badRequest().body(createErrorResponse("Ошибка получения информации о токене"));
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> revokeUserTokens(
        ...
        try {
            ...
            if (!hasPermissionToRevokeTokens(userId, authentication)) {
                return ResponseEntity.status(403).body(createErrorResponse("Недостаточно прав для отзыва токенов"));
            }
