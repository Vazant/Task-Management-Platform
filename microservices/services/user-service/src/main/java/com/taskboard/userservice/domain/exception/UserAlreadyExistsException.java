 ...
 *
 * <p>This exception is thrown when trying to create a new user with credentials (username or email)
 * that are already in use by another user in the system. It helps maintain data integrity and
 * prevents duplicate user accounts.
 *
 * <p>This exception should be thrown when:
 *
 * <ul>
 *   <li>Attempting to create a user with an existing username
 *   <li>Attempting to create a user with an existing email
 *   <li>Any operation that would result in duplicate user credentials
 * </ul>
 *
